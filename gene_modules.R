...
# library(MAST) !MAST is required to be installed separatelly, Seurat calls it
...
find_markers_parallel <- function(seurat_obj, cluster_list, ncores=4) {
  ...
  Markers <- bplapply(cluster_list, find_marker_wrapper,
...
cluster_cells <- function(seurat_obj, genes_to_remove, n = 1000) {
  ...
  # select genes and center the expression
  ...
  expression_center = t(scale(t(seurat_obj@data[var_genes, ]), 
                              center = T, scale = F))
  # calculate distance
...
create_signatures_single_core <- function(seurat_obj, cluster_list, logfold = 0.58,
                                          padj = 0.001, n_sig = 50) {
  ...
  for(i in 1:length(cluster_list)) {
    ...
    signature = FindMarkers(seurat_obj, ident.1 = cluster_cells,
                            min.pct = 0.2, min.cells.gene  = 10,
                            test.use = "MAST", only.pos = T, print.bar = F)
    # test if data frame is not empty or few rows
...
create_signatures <- function(signatures, logfold = 0.58, 
                              padj = 0.001, n_sig = 50) {
  # process the output of find_markers_parallel() to filter out genes and
  # create a list of signatures
  # Input: list of dataframes of markers, output of find_markers_parallel
  # Returns: list of signatures passing the criteria
  
  signatures_list = list()
  # find markers for all clusters
  cluster_names = names(signatures)
...
filter_cluster_signatures <- function(cluster_list, signatures_list,
  ...
  # size of signature for those which overlap > 0.75
  ...
  cluster_overlap = sapply(cluster_list, 
                           function(x) sapply(cluster_list, function(y) jaccardSets(x,y)))
  ...
  for(i in 1:nrow(cluster_overlap)) {
    ...
    if(length(row_jacc) >= 1) {
      ...
      # if any of sig_sizes is bigger than cluster_test, add cluster_test to exclude
      if(any(sig_sizes > cluster_sig_size)) {
        clusters_to_remove = append(clusters_to_remove, cluster_test)
      }
    }
  }
...
#cell_cluster = cluster_cells(kura_T10_g1) # testing for T10_g1
cell_cluster = cluster_cells(kura_all, genes_to_remove) # testing for T10_g1
#plot(cell_cluster, label = F)

# 2. Cutree
cat("Step2: Getting clusters\n")
#cluster_size = ncol(kura_T10_g1@data) # testing for T10_g1
cluster_size = ncol(kura_all@data) # testing for T10_g1
...
#signatures = find_markers_parallel(kura_T10_g1, cluster_list, ncores = 4)

# 5. create signatures
cat("Step5: Creating signatures\n")
#signature_list = create_signatures(signatures)
signature_list = create_signatures_single_core(kura_all, cluster_list)
