...
counts = count_table[, 4:ncol(count_table)]
# metadata
columnData = data.frame(condition = rep(c("C", "T5", "T10", "T20", "T40", "T80", "T160", "T320"), 2),
                        batch = c(rep("b1", 8), rep("b2", 8)))
# for including persisters
# metadata
...
# DESEq obj. For persisters, ignoring batch...
dds <- DESeqDataSetFromMatrix(countData = counts, 
                              colData = columnData,
                              design = ~ condition)
...
setwd("/path/kuramochi_olaparib_evolution/atac-seq/deseq2_peaks/homer_enrichment_down/")
setwd("/path/kuramochi_olaparib_evolution/atac-seq/deseq2_peaks/homer_enrichment_up/")
...
##################################
# The official barplots are here #
##################################
...
##########################################################
# Testing overlap across peaks for up and down regulated #
##########################################################

library(UpSetR)

# Plot upsetPlot comparing the numbers across states relative to ctrl.
upset(fromList(peaks_up), sets = c("T5_up", "T10_up", "T20_up", "T40_up",
                                   "T80_up", "T160_up", "T320_up"),
      order.by = "freq", keep.order = TRUE, mainbar.y.label = "Peak intersections")

upset(fromList(peaks_down), sets = c("T5_down", "T10_down", "T20_down", "T40_down",
                                     "T80_down", "T160_down", "T320_down"),
      order.by = "freq", keep.order = TRUE, mainbar.y.label = "Peak intersections")

# Use the pairwise jaccard distances across time points to measure the 
# similarity indices

# take upper triangle of the matrix
down_jaccard = peaks_down_overlap[upper.tri(peaks_down_overlap)]
up_jaccard = peaks_up_overlap[upper.tri(peaks_up_overlap)]

data_jac = data.frame(index = c(down_jaccard, up_jaccard), 
                      group = c(rep("down", length(down_jaccard)), 
                                rep("up", length(up_jaccard))))

library(ggsignif)
ggplot(data_jac, aes(x = group, y = index, fill = group)) +
  geom_boxplot() +
  geom_signif(comparisons = list(c("down", "up")), map_signif_level = T,
              test = c("wilcox.test")) +
  scale_fill_manual(values = c("#6baed6", "#a54d69"), name = "") +
  xlab("") +
  ylab("Jaccard index") +
...
# testing without 320, because it gained a lot of peaks
test_down = peaks_down_overlap[1:6, 1:6]
test_up = peaks_up_overlap[1:6, 1:6]

test_down_jaccard = test_down[upper.tri(test_down)]
test_up_jaccard = test_up[upper.tri(test_up)]

boxplot(test_down_jaccard, test_up_jaccard)


# for pairwise jaccard indices
peaks_up_overlap = sapply(all_up_peaks, 
                          function(x) sapply(all_up_peaks,
                                             function(y)bayesbio::jaccardSets(x,y)))
# for pairwise jaccard indices
peaks_down_overlap = sapply(all_down_peaks, 
                            function(x) sapply(all_down_peaks,
                                               function(y)bayesbio::jaccardSets(x,y)))


peaks_dist = as.dist(1 - cor(peaks_up_overlap))
cormM = cor(peaks_up_overlap, method="pearson")
dend = as.dendrogram(hclust(peaks_dist, method = "average"))
dend <- dendextend::click_rotate(dend, continue = TRUE)
