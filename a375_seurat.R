...
cells_cmo = read.csv("path/tag_calls_per_cell.csv")
...
# Read 10X matrix
a375_R1 = Read10X("path/sample_filtered_feature_bc_matrix")
...
a375_umap_sample_plot = ggplot(a375_g1_umap_sample$data,
                               aes(y = UMAP_2, x = UMAP_1, color = orig.ident)) +
  geom_point(size = 0.5) +
...
library(dendextend)
dend = as.dendrogram(hclust(dist(cormM)), type = "average")
dend <- click_rotate(dend, continue = TRUE)
