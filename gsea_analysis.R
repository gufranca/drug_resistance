...
setwd("/path/pdx_analysis")


################################################################################
# Running pathway enrichment again to run SSGSEA only on those enriched pathways
# Otherwise it takes too long
################################################################################

# load pathways
# seurat object with batch corr. plus pathways and metadata
load("pdx_all_combined.rda")
load("gene_sets.rda")


diff_expression_gsea <- function(seurat_obj, 
                                 ctrl = F,
                                 ctrl_id = c("V3", "V4"), rest_no_ctrl = T,
                                 cluster_column = 6) {
...
list_diff_exp_all_pdx_per = diff_expression_gsea(pdx_all_combined)
save(list_diff_exp_all_pdx_per, 
     file = "list_diff_exp_tumors_per_for_gsea.rda")

load("list_diff_exp_tumors_for_gsea.rda")
